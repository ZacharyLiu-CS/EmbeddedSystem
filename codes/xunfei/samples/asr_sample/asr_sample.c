...
int run_asr(UserData *udata)
{
	...
	if (NULL != rec_rslt){
		...
		tree = mxmlLoadString(NULL,rec_rslt,MXML_TEXT_CALLBACK); 
		node = mxmlFindElement(tree, tree, "rawtext",NULL, NULL,MXML_DESCEND);
		char *result = node->child->value.text.string;
		char tmp[2048];
		strcpy(tmp,result);
		printf("解析结果：[%s]\n",result);
		FILE * fp;
		fp = fopen("/home/gec210/VoiceResult/result.txt","w");
		fprintf(fp,"%s",tmp);
		fclose(fp);
	}
	else
...
{
	const char *login_config    = "appid = 5ac194bf"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
